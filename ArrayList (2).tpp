...
for (int i = 0; i < this->length - 1; i++) {
        for (int j = 0; j < this->length - 1 - i; j++) {
            // one key comparison between neighboring elements
            numComps++;
...
for (int i = 0; i < this->length - 1; i++) {
        ...
        // find index of smallest element from i+1 to end
        ...
        // swap current position with found minimum (if different)
        if (minIndex != i) {
            swap(i, minIndex);
            numSwaps++;
        }
...
for (int i = 1; i < this->length; i++) {
        T key = buffer[i];   // element to insert
        int j = i - 1;
        ...
        while (j >= 0) {
            numComps++;
            if (buffer[j] > key) {
                buffer[j + 1] = buffer[j];  // shift right
                numSwaps++;                 // count this move as a "swap"
                j--;
            }
            else {
                break;
